...
void main (void)
{
    ...
    if ( 4.0 * q * (q + cx) >= cy2 )
    {
        ...
        if ( cxp12 + cy2 >= 0.0625 )
        {
            ...
            for ( i = 0; i < FIX_ITERATION && dot(z, z) < 4.0; i ++)
            {
...
#ifdef ENABLE_PERIODICITY_CHECK
            if ( periodicityCheckFailed == false && i < FIX_ITERATION + 1 )
#else
            if ( i < FIX_ITERATION + 1 )
#endif
            {
                //Normalized Iteration Count to get a smoother image
                //smooth iter = iter + ( log(log(bailout)-log(log(cabs(z))) )/log(2)

                s.x = (float(i) - log(log(float(dot(z, z)))/ 2.0) / log2) / float(FIX_ITERATION);
